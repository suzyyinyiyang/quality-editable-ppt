...
## Direct Image Generation Invocation — No Tool / Environment Precheck

Before Gate 3, do not run any tool-availability precheck, environment scan, tool enumeration, package check, or tool-registry lookup. Directly invoke the explicitly selected image-generation route. If it cannot be invoked, stop before Gate 3 and ask the user to enable or select Image Gen / image2. Do not discover fallback tools, and do not use local SVG/PNG/canvas/script fallback as the high-visual preview.
