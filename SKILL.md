...
### 1. Direct Image Generation Invocation — No Tool / Environment Precheck

Before Gate 3, do **not** perform any tool-availability precheck, tool enumeration, environment scan, package check, tool-registry lookup, or discovery of unrelated system tools.

After Gate 2 confirmation, directly invoke the image-generation route that is explicitly available to the current task, such as:
...
- `Product Design` only when the user has explicitly selected it as the image-generation route;
- `Superpowers` only when the user has explicitly selected it as the image-generation route.

If no real image-generation route can be directly invoked, stop before Gate 3 and tell the user to enable or select Image Gen / image2. Do not try to discover fallback tools, do not inspect unrelated system tools, and do not create a local SVG / PNG / canvas / HTML / script fallback as a high-visual preview.
...
2. Equivalent image-generation route explicitly selected by the user in the current conversation.
3. Configured image-generation API.
4. SVG/HTML/canvas only for non-Gate-3 support assets after user approval, not as the high-visual preview fallback.
